...
export function ReleaseDetailPage() {
  ...
  const handlePlayTrack = (trackIndex: number) => {
    ...
    if (!selected || selected.published === false || !selected.audio_url) return;
  ...
  const hasPlayableTrack = (release.tracks ?? []).some((track) => track.published !== false && Boolean(track.audio_url));

  return (
    <div className="page-section release-detail editorial-detail">
      <header className="detail-header">
        <div className="detail-cover music-cover"><img src={release.artwork_url ?? ''} alt={release.title} /></div>
        <div className="detail-copy">
          <p className="eyebrow">{release.type} / {release.release_date.slice(0, 4)}</p>
          <h1>{release.title}</h1>
          <p>{release.description}</p>
          <div className="detail-actions"><button type="button" className="button primary" onClick={() => handlePlayTrack((release.tracks ?? []).findIndex((track) => track.published !== false && Boolean(track.audio_url)))} disabled={!hasPlayableTrack}>Play release</button><Link to={publicRoutes.releases} className="text-link">Back to archive</Link></div>
          <ul className="platform-list">{(release.platform_links ?? []).map((platform) => <li key={platform.id}><a href={platform.url} target="_blank" rel="noreferrer">{platform.label}</a></li>)}</ul>
        </div>
      </header>
      <section className="tracklist-block">
        <div className="section-heading"><p className="eyebrow">Tracklist / {String((release.tracks ?? []).length).padStart(2, '0')} tracks</p><h2>Listen closely</h2></div>
        <ol className="tracklist">{(release.tracks ?? []).map((track, index) => { const playable = track.published !== false && Boolean(track.audio_url); return <li key={track.id} className={`track-row ${playable ? '' : 'track-row--disabled'}`}><button type="button" className="track-play" onClick={() => handlePlayTrack(index)} aria-label={`Play ${track.title}`} disabled={!playable}>{playable ? '▶' : '•'}</button><div className="track-info"><span className="track-index">{String(index + 1).padStart(2, '0')}</span><span>{track.title}</span></div><span>{formatTime(track.duration)}</span></li>; })}</ol>
      </section>
    </div>
  );
}
