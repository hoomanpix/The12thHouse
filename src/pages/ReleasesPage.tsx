...
export function ReleasesPage() {
  ...
  return (
    <div className="page-section archive-page">
      ...
      <div className="archive-toolbar"><span className="eyebrow">Filter</span><div className="filter-bar" role="group" aria-label="Release filters">{options.map((option) => <button key={option.value} type="button" className={filter === option.value ? 'filter-pill active' : 'filter-pill'} aria-pressed={filter === option.value} onClick={() => setFilter(option.value)}>{option.label}</button>)}</div><span className="archive-count" role="status" aria-live="polite">{String(filteredReleases.length).padStart(2, '0')} works</span></div>
      <div className="archive-list">{filteredReleases.map((release, index) => <Link className={`archive-item ${release.status === 'upcoming' ? 'archive-item--future' : ''}`} to={`/releases/${release.slug}`} key={release.id}><span className="archive-item__number">{String(index + 1).padStart(2, '0')}</span><span className="archive-item__image music-cover"><img src={release.artwork_url ?? ''} alt="" loading="lazy" decoding="async" width="76" height="76" /></span><span className="archive-item__title">{release.title}</span><span className="archive-item__type">{release.status === 'upcoming' ? 'upcoming' : release.type}</span><span className="archive-item__date">{release.release_date.slice(0, 4)}</span></Link>)}</div>
    </div>
  );
}
